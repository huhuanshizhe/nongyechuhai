...
import {
  getAllSuppliersWithCredentials,
...
function getCopy(locale: string) {
  ...
  return {
    ...
    heroTitle: isZh
      ? '农产品出海，信任是第一生产力'
      : 'In agricultural export, trust is the primary currency',
    ...
    noCerts: isZh
      ? '该供应商认证信息正在收集中。'
      : 'Certification information for this supplier is being collected.',
    noScenes: isZh
      ? '养殖/种植场景图片正在收集中。'
      : 'Farming and cultivation scene images are being collected.',
    ...
    certTypeLabels: {
      ...
      EXPORT_FOOD_REGISTRATION: isZh
        ? '出口食品生产企业备案'
        : 'Export Food Production Registration',
      GAP: 'GAP',
      HACCP: 'HACCP',
      ISO22000: 'ISO 22000',
...
function CertBadge({
  ...
  return (
    <article
      className={`credential-badge${isValid ? '' : ' credential-badge--expired'}`}
    >
      ...
      <div className="credential-badge__body">
        <strong>{name}</strong>
        {issuingBody && (
          <span className="credential-badge__issuer">{issuingBody}</span>
        )}
        {certificateNumber && (
        ...
        <div className="credential-badge__dates">
          {issuedAt && (
            <span>
              {isZh ? '发证: ' : 'Issued: '}
              {new Date(issuedAt).toLocaleDateString(isZh ? 'zh-CN' : 'en-US', {
                year: 'numeric',
                month: 'short',
              })}
            </span>
...
function SupplierCredentialsSection({
  supplier,
  locale,
}: {
    ...
    <section className="credential-supplier" data-rise="true">
      <div className="credential-supplier__head">
        <div>
          ...
          {supplier.supplierDescription && (
            <p className="credential-supplier__desc">
              {supplier.supplierDescription}
            </p>
          )}
          <div className="credential-supplier__meta">
            {supplier.supplierCountry && (
              <span className="catalog-chip">
                {[supplier.supplierCountry, supplier.supplierCity]
                  .filter(Boolean)
                  .join(', ')}
              </span>
            )}
      ...
      <div className="credential-section">
        ...
        <h3>
          {isZh
            ? '国际第三方合规认证'
            : 'International Third-Party Compliance Certifications'}
        </h3>
...
export default async function TraceabilityPage({
  ...
  return (
    <main className="page-shell">
      <aside className="hp-notice">
        <strong>
          {locale === 'zh' ? '原有业务供应商资料' : 'Legacy supplier records'}
        </strong>
        <p>
          {locale === 'zh'
            ? '以下为原有业务记录，不代表高原项目已完成供应商核验。'
            : 'These are legacy business records, not completed supplier verification for the plateau project.'}
        </p>
        <Link href="/traceability">
          {locale === 'zh'
            ? '返回高原质量流程 →'
            : 'Back to plateau quality process →'}
        </Link>
      </aside>
      {/* Hero */}
        ...
        <section className="section-card" data-rise="true">
          <div className="section-head">
            <h2>
              {locale === 'zh'
                ? '供应商资质信息正在收集中'
                : 'Supplier credential information is being collected'}
            </h2>
